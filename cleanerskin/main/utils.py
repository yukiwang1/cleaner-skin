...
def search(query):
    ...
    for keyword in search_query:
        ...
        dbQuery = db.engine.execute("select * from Product where brand ilike %s", ("%" + keyword + "%"))
        ...
        if brand_results:
            ...
            if results:
                ...
                dbQuery = db.engine.execute("select * from Product where name ilike %s", ("%" + keyword + "%"))
...
def query_user_or_product(_type, identifier):
    ...
    return abort(404, f'This {_type} does not exist, try again') if (not user and not product) else (user or product)
...
def review_product(ingredients):
    ...
    for ingredient in ingredients:
        ...
        result = SafeIngredients.query.filter(SafeIngredients.names.ilike("%" + ingredient + "%")).all()
        if result: 
            result = []
            continue

        # check entire phrase
        result = BadIngredients.query.filter(BadIngredients.names.ilike("%" + ingredient + "%")).all()
        ...
        for keyword in ingredient_keywords:
            ...
            # check false positives
            result = SafeIngredients.query.filter(SafeIngredients.names.ilike("%" + keyword + "%")).all()
            if result: 
                result = []
                break
            

            result = BadIngredients.query.filter(BadIngredients.names.ilike("%" + keyword + "%")).all()
